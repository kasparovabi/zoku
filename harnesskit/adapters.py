...
def _adapt_cursor(config: HarnessConfig) -> AdaptResult:
    ...
    # Modern .cursor/rules format (.mdc) — the recommended approach
    mdc_parts: list[str] = [
        "---",
        f"description: \"{config.project_name} harness rules\"",
        "globs: [\"**/*\"]",
        "alwaysApply: true",
        "---",
        "",
    ]
    mdc_parts.extend(parts)
...
def _adapt_codex_cli(config: HarnessConfig) -> AdaptResult:
    """Generate ``.codex/config.toml`` for OpenAI Codex CLI.

    Codex CLI uses TOML configuration with model, provider, and feature settings.
    Config resolution: project (.codex/) > user (~/.codex/) > system (/etc/codex/).
    """
    ...
    model_name = config.model.name or "o4-mini"
    provider = config.model.provider or "openai"

    # Build TOML config
    toml_lines: list[str] = [
        f"# HarnessKit configuration for: {config.project_name}",
        f"# Generated by HarnessKit v{_version()}",
        "",
        f'model = "{model_name}"',
        f'provider = "{provider}"',
    ...
    if instructions_parts:
        ...
        toml_lines.append(f'instructions = "{instructions}"')
        toml_lines.append("")

    # Also generate a legacy codex.json for backwards compatibility
    codex_json_data: dict = {
        "model": model_name,
        "approval_mode": approval_mode,
    }
    if instructions_parts:
        codex_json_data["instructions"] = "\n".join(instructions_parts)
    codex_json = json.dumps(codex_json_data, indent=2, ensure_ascii=False)
