"""Phantom Agent hook handlers for Claude Code.
...
Usage from shell scripts::
    ...
    echo "$INPUT" | python -m phantom.hooks post-tool-use
    echo "$INPUT" | python -m phantom.hooks stop
    echo "$INPUT" | python -m phantom.hooks session-start
"""
...
def handle_stop(event: dict) -> dict:
    ...
    lines = [
        "[Phantom Agent] New workflow patterns discovered:",
        "",
    ]
    ...
    lines.append("Run `python -m phantom patterns` to see all discovered workflows.")
...
def handle_session_start(event: dict) -> dict:
    ...
    lines = [
        f"[Phantom Agent] {len(patterns)} workflow pattern(s) detected from your previous sessions:",
        "",
    ]
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: python -m phantom.hooks <post-tool-use|stop|session-start>",
              file=sys.stderr)
        sys.exit(1)
