...
def handle_stop(event: dict) -> dict:
    """Analyse all recorded traces and save new patterns silently.

    Stop hooks don't support hookSpecificOutput, so pattern discovery
    runs silently. Users see patterns on next SessionStart or via CLI.
    """
    cwd = event.get("cwd")
